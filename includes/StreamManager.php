...
class StreamManager {
    ...
    public function getEvent($event_id) {
        $stmt = $this->db->prepare("SELECT * FROM streaming_events WHERE id = ?");
        $stmt->execute([$event_id]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    public function getEventByKey($stream_key) {
        $stmt = $this->db->prepare("SELECT * FROM streaming_events WHERE stream_key = ?");
    ...
    public function generateHlsToken($stream_key, $client_ip = null) {
        try {
            ...
            $stmt = $this->db->prepare("
                ...
                VALUES (?, ?, ?, ?)
            ");
            $stmt->execute([$stream_key, $token, $client_ip, $expires]);
            return [
                'token' => $token,
                'expires_at' => $expires,
    ...
    public function validateToken($token) {
        try {
            $stmt = $this->db->prepare("SELECT * FROM hls_tokens WHERE token = ? AND expires_at > NOW()");
            $stmt->execute([$token]);
            $token_record = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!$token_record) {
                return null;
            }
            $stmt = $this->db->prepare("UPDATE hls_tokens SET used_at = NOW() WHERE id = ?");
    ...
    public function updateEventStatus($event_id, $status) {
        try {
            ...
            $stmt = $this->db->prepare("
                ...
                WHERE id = ?
            ");
            return $stmt->execute([$status, $started_at, $ended_at, $event_id]);
        } catch (PDOException $e) {
    ...
    public function deleteEvent($event_id) {
        try {
            ...
            $stmt = $this->db->prepare("DELETE FROM hls_tokens WHERE stream_key = ?");
            $stmt->execute([$event['stream_key']]);
            $stmt = $this->db->prepare("DELETE FROM streaming_events WHERE id = ?");
    ...
    private function generateStreamKey() {
        do {
            ...
            $stmt = $this->db->prepare("SELECT COUNT(*) as count FROM streaming_events WHERE stream_key = ?");
            $stmt->execute([$key]);
    ...
    public function cleanupExpiredTokens() {
        try {
            $stmt = $this->db->prepare("DELETE FROM hls_tokens WHERE expires_at < NOW()");
            return $stmt->execute();
        } catch (PDOException $e) {
    ...
    public function getActiveStreamsCount() {
        $stmt = $this->db->prepare("SELECT COUNT(*) as count FROM streaming_events WHERE status IN ('pending', 'live')");
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
